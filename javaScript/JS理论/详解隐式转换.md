之前看了js高级3的关于隐式转换讲解，看着那一堆的规定感觉是懂了，但是自己一套用那些规则发现不对，越来越蒙
...
值显然没有多大意义。（不管是在+运算符还是==运算符）还不如转换为字符串格式的日期，所以默认Date类型会优先进行toString转换。故有以上的规则：PreferredType没有设置时，Date类型的对象，PreferredType默认设置为String，其他类型对象PreferredType默认设置为Number。
...
### 4、== 运算符隐式转换
```
比较运算 x==y, 其中 x 和 y 是值，这样的比较按如下方式进行：
...
1、若 Type(x) 与 Type(y) 相同， 则
    ...
    3* 若 Type(x) 为 Number， 则
        ...
        (1)、若 x或y 为 NaN， 返回 false。
        (2)、若 x 与 y 为相等数值， 返回 true。
        (3)、若 x 为 +0 且 y 为 −0，或者x 为 −0 且 y 为 +0, 返回 true。
        (4)、返回 false。
        
    4* 若 Type(x) 为 String, 则当 x 和 y 为完全相同的字符序列（长度相等且相同字符在相同位置）时返回 true。 否则， 返回 false。
    5* 若 Type(x) 为 Boolean, 当 x 和 y 为同为 true 或者同为 false 时返回 true。 否则， 返回 false。
    6*  当 x 和 y 为引用同一对象时返回 true。否则，返回 false。
2、若 Type(x) 与 Type(y) 不相同
    1、若 x 为 null 且 y 为 undefined， 返回 true。
    2、若 Type(x) 为 Number 且 Type(y) 为 String，返回比较 x == ToNumber(y) 的结果。
    3、若 Type(x) 为 Boolean， 返回比较 ToNumber(x) == y 的结果。
    4、若 Type(x) 为 String 或 Number，且 Type(y) 为 Object，返回比较 x == ToPrimitive(y) 的结果。
    5、返回 false。
```
总结：
1.类型相同时，没有类型转换
...
3、根据上面第3条，等式右边y = ToNumber(false) = 0。结果变成 [] == 0。
4、按照上面第4条，比较变成ToPrimitive([]) == 0。
    按照上面规则进行原始值转换，[]会先调用valueOf函数，返回this。
   不是原始值，继续调用toString方法，x = [].toString() = ''。
   故结果为 '' == 0比较。
5、根据上面第2条，等式左边x = ToNumber('') = 0。
...
```
1、当执行a == 1 && a == 2 && a == 3 时，会从左到右一步一步解析，首先 a ==  1，会进行上面第4步转换。ToPrimitive(a， Number) == 1。
