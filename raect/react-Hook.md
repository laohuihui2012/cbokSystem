...
规则：只能在函数最外层调用 Hook。不要在循环、条件判断或者子函数中调用。
...
3.尽可能写把组件写成无状态组件，因为它们更方便复用，可独立测试,并且性能会好点（性能差异几乎可以忽略不计，因为闭包和类的原始性能只有在极端场景下才会有明显的差别。）。。但是当需要改动需要将组件变成有状态组件之后 又得重新写class组件
4.class 不能很好的压缩，并且会使热重载出现不稳定的情况。
```
...
##### 使用单个还是多个 state 变量
刚开始使用的时候，我在纠结是使用一个state还是多个，是不是使用一个性能会好点？
可以使用多个，也可以单独拆分一个个得。但是我们最好是找到平衡点，这样有利于后期逻辑的抽取，还有一些其他的操作（例如需要监听某一个变量的变化而更新组件，如果只有一个state，其他变量的变化也会引起组件的更新）
一个state：
...
所以react规定我们必须把hooks写在函数的最外层，不能写在ifelse等条件语句当中，来确保hooks的执行顺序一致。如果在判断条件中来声明，就会造成数组中使得执行顺序错误
```
...
##### 怎么跳过一些不必要的副作用函数
每次重新渲染都要执行一遍这些副作用函数，显然是不合理。怎么跳过一些不必要的计算呢？我们只需要给useEffect传第二个参数即可。用第二个参数来告诉react只有当这个参数的值发生改变时，才执行我们传的副作用函数（第一个参数）。
...
```
有三种情况：
1.当第二个参数什么都不传，表示每次渲染更新都会执行（很多时候这样会影响性能）
2.当我们第二个参数传一个空数组[]时，其实就相当于只在首次渲染的时候执行。也就是componentDidMount加componentWillUnmount的模式
3.当第三个我们传入useEffect使用到的变量，只有当这个变量变化的时候才会执行，所以很多时候我们可以利用这个做性能优化。列示class组件中下面的写法
